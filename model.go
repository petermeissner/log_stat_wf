...
func (ls *LogStat) String() string {
	return fmt.Sprintf("ID:%d | Host:%-10s | Start:%s | Interval:%ds | Level:%-8s | Logger:%-30s | Count:%d",
		ls.ID, ls.HostName, ls.TS_Start, ls.TS_Interval_S, ls.Level, ls.Logger, ls.N)
}
